...
public abstract class TrayApplication : IDisposable {
    ...
    protected TrayApplication(TrayApplicationOptions? options = null, IUpdateService? updateService = null) {
    ...
    protected virtual nint GetTrayIconHandle() {
        ...
        var path = Path.GetFullPath(_options.IconPath);
        var icon = NativeMethods.LoadImage(0, path, NativeMethods.IMAGE_ICON, 0, 0,
            NativeMethods.LR_LOADFROMFILE | NativeMethods.LR_DEFAULTSIZE);
        if (icon == 0)
            throw new Win32Exception(Marshal.GetLastWin32Error(), $"Unable to load tray icon: {path}");
        _ownsIcon = true;
        return icon;
    }
    protected void Exit() {
    ...
    protected Task<UpdateCheckResult?> CheckForUpdatesAsync(CancellationToken cancellationToken = default) =>
        _updateService is null ? Task.FromResult<UpdateCheckResult?>(null) : CheckCoreAsync(cancellationToken);
    ...
    private void CreateMessageWindow() {
        ...
        _windowHandle = NativeMethods.CreateWindowEx(
            0, className, _options.ApplicationName, 0, 0, 0, 0, 0, 0, 0, module, 0);
        if (_windowHandle == 0) {
    ...
    private nint WindowProcedure(nint hWnd, uint msg, nuint wParam, nint lParam) {
        switch (msg) {
            ...
            case NativeMethods.WM_TRAYICON:
                HandleTrayMessage((uint)((nuint)lParam & 0xFFFF));
                return 0;
    ...
    private void ShowContextMenu() {
        ...
        try {
            ...
            NativeMethods.TrackPopupMenu(menu, NativeMethods.TPM_RIGHTBUTTON | NativeMethods.TPM_BOTTOMALIGN, point.X,
                point.Y, 0, _windowHandle, 0);
        } finally {
